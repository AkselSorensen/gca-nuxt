      ...
      <div v-else class="purchases-list anim-up">
        <article v-for="item in purchases" :key="item.order_item_id" class="purchase-card">
          ...
          <div class="purchase-actions">
            <button v-if="item.files?.length" class="btn-dl-main" @click="download(item.order_item_id)" :disabled="downloading === item.order_item_id">
...
function downloadInvoice(orderItemId: number) {
  // Loader stylé ~5s pendant la génération, puis téléchargement direct.
  // PAS de window.open : les navigateurs bloquent les popups hors geste
  // utilisateur (about:blank muet). Le serveur répond Content-Disposition:
  // attachment → la navigation déclenche le téléchargement SANS quitter la page.
  ...
  invoiceTimer.value = window.setInterval(() => {
    ...
    if (invoiceProgress.value >= 100) {
      ...
      window.location.href = base + '/api/invoice/' + orderItemId
...
.purchase-actions { display: flex; flex-direction: column; gap: 10px; justify-content: center; flex-shrink: 0; }
.purchase-actions .btn-dl-main, .purchase-actions .btn-invoice { width: 100%; }
@media (max-width: 640px) { .purchase-actions { flex-direction: row; justify-content: stretch; } .purchase-actions .btn-dl-main, .purchase-actions .btn-invoice { flex: 1; } }
