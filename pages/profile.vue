            ...
            <template v-else-if="user?.sellerStatus === 'rejected'">
              ...
              <form class="seller-form" @submit.prevent="submitSellerRequest">
                ...
                <div class="field"><label>Discord</label>
                  <div v-if="user?.discordTag" class="discord-linked">
                    ...
                    <span>Discord lié — <strong>{{ user.discordTag }}</strong></span>
                  </div>
...
.btn-discord-link:hover { background:rgba(88,101,242,0.18);border-color:rgba(88,101,242,0.6); }
.discord-linked { display:flex;align-items:center;gap:8px;padding:11px 14px;border-radius:8px;border:1px solid rgba(110,231,183,0.25);background:rgba(110,231,183,0.06);color:var(--text-secondary);font-size:.85rem; }
.discord-linked strong { color:var(--green); }
